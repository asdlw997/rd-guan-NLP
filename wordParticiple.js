...
export class Tokenizer {
    constructor() {
        this.input = [];//inputId说明,  0:str(字符串) 1:dictionary(词典) 
    ...
    formWordStr(word, wordId, dictionary) {//在词典的词的输出格式
        return {
            type: dictionary.findPartWordList(wordId)[0],
            data: word
            /*,  methodObj:dictionary.findMethodObjList(wordInfos[1])[0] */
        }
    }

    findLongestWord(str, startIndex, dictionary) {
        let word = "";
        let wordId = null;
        ...
        for (let nowIndex = startIndex; nowIndex < str.length; nowIndex++) {
            ...
            nodeId = dictionary.findNextNodeId(character, nodeId);
            if (nodeId === null) {
                break;
            } else {
                if (dictionary.isWordEnd(nodeId)) {
                    word = _str + character;
    ...
    findAWord(str, startIndex, dictionary, result) {//返回[word, wordId, wordEndIndex(在字符串中)]
            return this.findLongestWord(str, startIndex, dictionary);
    }
   
    splitString(str, dictionary) {
        let nowIndex = -1;
        let noWordStr = "";
        let result = [];
        while (nowIndex < str.length) {
            nowIndex++;
            let character = str[nowIndex];
            if (dictionary.isCharacterWordHead(character)) {
                let wordInfos = this.findAWord(str, nowIndex, dictionary, result);
                if (wordInfos[1] !== null) {
                    if (noWordStr !== "") result.push(this.formNoWordStr(noWordStr));//
                    result.push(this.formWordStr(wordInfos[0],wordInfos[1], dictionary));
    ...
    run() {
        this.output[0]=this.splitString.apply(this, this.input);
    }
}
